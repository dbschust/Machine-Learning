...
def calc_gradients(img):
   ...
   Ix and Iy matrices.
   """
   gx = cv2.filter2D(img, -1, DOG_GX, borderType=cv2.BORDER_CONSTANT)
   gy = cv2.filter2D(img, -1, DOG_GY, borderType=cv2.BORDER_CONSTANT)
   return gx, gy
...
def display_flow(Vx, Vy, title="flow"):
   """
   ...
   Vx_vis = cv2.normalize(Vx, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
   Vy_vis = cv2.normalize(Vy, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
...
def overlay_flow(Vx, Vy, img, title, alpha=0.6):
   ...
   #blend original and flow images
